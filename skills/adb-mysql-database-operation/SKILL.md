...
包含 **10 大功能模块**：集群管理、资源组管理、数据库管理、网络管理、账号管理、安全管理、备份恢复、监控管理、SQL 诊断、空间分析。

## ⚠️ 重要：前置条件和网络配置

### 前置条件（必须）

在调用 CreateDBCluster API 之前，**必须先创建以下服务关联角色**：

#### 1. Privatelink 服务关联角色
```bash
# 服务名: privatelink.aliyuncs.com
# 角色名: AliyunServiceRoleForPrivatelink
```

#### 2. AnalyticDB for MySQL 服务关联角色
```bash
# 服务名: ads.aliyuncs.com
# 角色名: AliyunServiceRoleForAnalyticDBForMySQL
```

#### 创建服务关联角色的 API 调用
```javascript
async function createServiceLinkedRole(serviceName) {
  const apiParams = {
...
// 创建两个必需的服务关联角色
async function setupServiceLinkedRoles() {
  console.log('创建 Privatelink 服务关联角色...');
  await createServiceLinkedRole('privatelink.aliyuncs.com');
  
  console.log('创建 AnalyticDB for MySQL 服务关联角色...');
  await createServiceLinkedRole('ads.aliyuncs.com');
  
  console.log('服务关联角色创建完成！');
}
```

**⚠️ 注意**：缺少服务关联角色是导致 `InvalidVPC.NotFound` 错误的主要原因！
...
const result = await adb.clusters.createCluster({
  ...
  zoneId: 'cn-hangzhou-h',
  payType: 'Postpaid',
  commodityCode: 'ads_post',
  executorCount: '2',
  diskCategory: 'cloud_effd',
  diskSize: '500',
  description: '湖仓版测试集群'
});
...
| 模式 | DBClusterCategory | 必填参数 |
|------|-------------------|----------|
| flexible | MixedStorage | ComputeResource |
| reserver | Cluster | DBClusterClass + DBNodeGroupCount + DBNodeStorage |

##### 存储容量限制

- **C8 规格**: 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 2000 (GB)
- **C32 规格**: 100-8000 (GB), 步长 100
...
| 错误码 | 错误信息 | 原因 | 解决方案 |
|--------|----------|------|----------|
| InvalidVPC.NotFound | VPC or VSwitch is not valid | 服务关联角色未创建 | 先创建 `AliyunServiceRoleForPrivatelink` 和 `AliyunServiceRoleForAnalyticDBForMySQL` |
| MissingDBClusterVersion | Missing DBClusterVersion | 缺少版本参数 | 添加 `DBClusterVersion: '3.0'` |
| MissingMode | Missing Mode | 缺少 Mode 参数 | 添加 `Mode: 'flexible'` 或 `Mode: 'reserver'` |
| InvalidParameter.TotalCoreCount | The total core num should be at least 2 | 预留模式节点数不足 | 确保 DBNodeGroupCount >= 2 |
| OperationDenied.RegionZoneNotSupport | Region/Zone not support | VPC与可用区不匹配 | 不指定 VPC，让系统自动分配 |
