---
name: self-hosted-redis-ops
description: 自建/他云Redis智能运维技能，支持基础的Redis功能诊断
---

# Self-Hosted Redis-Ops 参考文档
...
### 支持的功能
自建或其他云厂商的Redis实例，基于标准Redis协议，支持以下基础功能：
- **安全相关功能**: 安全异常事件、基线分析、敏感数据发现
- **实例管理**: 实例信息查询

### 不支持的功能
由于数据采集限制，自建/他云Redis不支持以下功能：
- CPU实时诊断和历史诊断
- 时延洞察解读
- Large Key查询分析
- 监控指标查询
...
- **实例权限**: 需要对目标自建Redis实例有管理权限
- **DAS Agent**: 需要开通DAS Agent服务并配置实例管理权限

## 注意事项
1. **标准协议**: 仅支持标准Redis协议，各厂商特有定制功能暂不兼容
2. **功能限制**: 功能受限主要是因为无法获取完整的性能监控数据和日志
3. **安全功能**: 基础的安全检测功能仍然可用