...
async def list_instance(
        search_key: Optional[str] = Field(default=None, description="Optional search key"),
        db_type: Optional[str] = Field(default=None, description="Optional dbType (e.g., mysql, polardb, oracle, "
                                                                 "postgresql, sqlserver, polardb-pg, etc.)"),
        env_type: Optional[str] = Field(default=None, description="Optional EnvType (e.g., product, dev, test, etc.)")
) -> List[InstanceDetail]:
    client = create_client()
